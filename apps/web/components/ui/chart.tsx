...
/**
 * Chart primitives — the shadcn/ui "chart" component, owned source, adapted to
 * recharts 3.9 + Tailwind v4 semantic tokens (bg-popover, text-muted-foreground,
 * border, etc.) and Coomander's `.dark` theming.
 *
 * `ChartContainer` wraps recharts' `ResponsiveContainer` and injects a scoped
 * `<style>` that maps each `ChartConfig` entry to a `--color-<key>` CSS variable
 * (per light/dark theme), so chart elements can reference `var(--color-<key>)`
 * for theme-aware fills/strokes. `ChartTooltipContent` / `ChartLegendContent`
 * are custom renderers passed to recharts' `Tooltip` / `Legend` via the `content`
 * prop (as cloned elements — recharts injects `active`/`payload`/`label` at
 * runtime, which is why those props are optional here).
 */
...
// Map a theme name to the CSS selector that scopes its variables.
...
/** A single tooltip payload entry as recharts injects it at runtime. */
type ChartTooltipItem = {
  ...
  color?: string;
  fill?: string;
  payload?: ({ fill?: string } & Record<string, unknown>) | undefined;
};
...
/** A single legend payload entry as recharts injects it at runtime. */
...
/** Resolve the ChartConfig entry that matches a tooltip/legend payload item. */
