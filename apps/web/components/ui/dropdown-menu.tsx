...
/**
 * Legacy controlled DropdownMenu API (props-driven).
 *
 * Backed by Radix DropdownMenu primitives — provides keyboard navigation,
 * collision-aware positioning, focus management, and ARIA wiring for free.
 *
 * For new code, prefer the compound primitive API:
 *   `<DropdownMenuRoot><DropdownMenuTrigger>...</DropdownMenuTrigger>...</DropdownMenuRoot>`
 *
 * @example
 * ```tsx
 * <DropdownMenu
...
export function DropdownMenu({
  ...
  return (
    <DropdownMenuRoot>
      ...
      >
        {items.map((item) => (
          <DropdownMenuItem key={item.key} danger={item.danger} onSelect={() => item.onClick()}>
            {item.icon}
            {item.label}
          </DropdownMenuItem>
        ))}
