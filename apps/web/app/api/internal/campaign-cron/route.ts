...
import { dispatchScheduledCampaigns, reconcileStuckCampaigns } from "@/jobs/index";
 ...
 * (#597, epic #595, sync #222) and reconciling stuck sends (hardening
 * follow-up, sync #222). A Workers `scheduled()` handler can't touch D1
 * directly (no request context), so it replays through the app's own fetch
 * pipeline, same pattern as app/api/internal/campaign-batch/route.ts.
 *
